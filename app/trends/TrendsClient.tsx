...
export default function TrendsPage({ initialVideos = [], initialComments = {} }: { initialVideos?: TrendVideo[]; initialComments?: Record<string, {comments: YTComment[]; demo: boolean; disabled?: boolean}> }) {
  ...
  const [videos, setVideos] = useState<TrendVideo[]>(initialVideos);
  const [demo, setDemo] = useState(!initialVideos.length);
  ...
  const loadComments = async (m: VideoMetrics) => {
    ...
    setCOpen(m.videoId);
    if (initialComments[m.videoId] && !cData[m.videoId]) {
      const result = initialComments[m.videoId];
      const analysis = analyzeComments(result.comments);
      setCData(p => ({...p, [m.videoId]: {loading:false, ...analysis, ...result, comments:[...result.comments].sort((a,b)=>b.likes-a.likes).slice(0,10)}}));
      return;
    }
