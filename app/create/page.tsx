...
const PROVIDERS: { id: HostAi; name: string; desc: string }[] = [
  { id: "chatgpt", name: "ChatGPT", desc: "창 열기 · 자동 입력·전송" },
  { id: "gemini", name: "Gemini", desc: "창 열기 · 자동 입력·전송" },
...
function CreateInner() {
  ...
  const generate = async () => {
    ...
    if (provider === "chatgpt" || provider === "gemini") {
      ...
      // 한 번의 클릭으로: 복사 시작 → 새 탭 열기 → 확장프로그램이 프롬프트 입력·전송.
      // 새 탭 열기는 사용자 클릭 이벤트 안에서 즉시 실행해야 팝업 차단을 피할 수 있다.
      const clipboardTask = navigator.clipboard.writeText(prompt).then(() => true).catch(() => false);
      openHostAi(provider, prompt);
      const copied = await clipboardTask;

      say(copied
        ? `${name} 창을 열었어요. 지시문을 자동 입력해 전송합니다.`
        : `${name} 창을 열었어요. 자동 전송이 안 되면 아래 ‘지시문 다시 복사’를 사용해 주세요.`);
      return;
    }
    ...
    <main className="ws-main">
      ...
      {step === 1 && <section className="ws-panel ws-compose">
        ...
        {showPaste && (provider === "chatgpt" || provider === "gemini") && <div className="ws-form-group ws-stack">
          <div className="ws-note"><UiIcon name="info" size={18} /><span>① {hostName} 창 열기 → ② 프롬프트 자동 입력·전송 → ③ 작성이 끝나면 결과를 복사해 아래로 가져오세요. 자동 입력은 당근 Post AI 확장프로그램이 담당해요.</span></div>
          <div className="ws-secondary-actions"><button type="button" className="ws-button ws-button-wide" onClick={() => void copy(hostPrompt)}><UiIcon name="copy" size={16} />지시문 다시 복사</button></div>
