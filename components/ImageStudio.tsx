  ...
  return <div className={styles.root}>
    ...
    <section className={styles.panel} aria-labelledby="image-settings-title">
      ...
      <fieldset disabled={busy} className={styles.fields}>
        ...
        <div className={styles.pair}><label>준비할 이미지<select value={Math.min(settings.count, 2)} onChange={e => changeCount(Number(e.target.value))}>{[1, 2].map(n => <option key={n} value={n}>{n}장{n === 1 ? " · 기본" : ""}</option>)}</select></label><label>생성 방식<select value={settings.method} onChange={e => updateSettings({ method: e.target.value as ImageSettings["method"] })}><option value="chatgpt">ChatGPT 창에서 생성</option><option value="server">사이트 AI로 자동 생성</option></select></label></div>
      </fieldset>
    ...
    <section className={styles.panel} aria-labelledby="image-gallery-title">
      ...
      <p className={styles.helper}>당근 소식 이미지는 기본 1장, 필요할 때 최대 2장까지 준비하세요.</p>
